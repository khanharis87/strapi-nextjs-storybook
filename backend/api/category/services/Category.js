...
module.exports = {
  ...
  fetchAll: (params, populate) => {
    ...
    const withRelated = populate || Category.associations
      .filter(ast => ast.autoPopulate !== false)
      .map(ast => ast.alias);

    const filters = convertRestQueryParams(params);

    return Category.query(buildQuery({ model: Category, filters }))
  ...
  count: (params) => {
    ...
    return Category.query(buildQuery({ model: Category, filters: _.pick(filters, 'where') })).count();
  },
